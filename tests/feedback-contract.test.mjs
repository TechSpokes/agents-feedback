...
test('public repository support files and release channels remain present', () => {
  ...
  const ciConfig = parseYaml(ciWorkflow);
  const releaseConfig = parseYaml(releaseWorkflow);
  assert.deepEqual(ciConfig.on.push.branches, ['main']);
  assert.ok(Object.hasOwn(ciConfig.on, 'pull_request'));
  assert.equal(ciConfig.concurrency['cancel-in-progress'], true);
  assert.match(ciConfig.concurrency.group, /github\.event\.pull_request\.number/);
  ...
  assert.equal(releaseConfig.concurrency['cancel-in-progress'], true);
  assert.match(releaseConfig.concurrency.group, /inputs\.tag/);
  ...
  assert.ok(dependabot, 'Dependabot configuration not discovered');
